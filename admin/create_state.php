...
$qry_validate = mysqli_query($con,"select * from `state` where `name`='$name' ") or die(mysqli_error($con));
if(mysqli_num_rows($qry_validate)>0)
{
$msg ='<center><span style="color:#ff0000;">state already exists<span></center>';
}
else
{

$insert = mysqli_query($con,"insert into `state`(`c_id`,`name`,`order`,`status`,`c_date`)values('$c_id','$name','$order','$status','$c_date')") or die(mysqli_error($con));

$msg ='<center><span style="color:#ff0000;">state insert successfully<span></center>';
}
...
$update = mysqli_query($con,"update `state` set `c_id`='$c_id',`name`='$name',`order`='$order',`status`='$status' where `id`='$_REQUEST[edit_id]' ")or die(mysqli_error($con));

//$msg ='<center><span style="color:#ff0000;">Admin Updated Successfully<span></center>';
?>
<script type="text/javascript">
//alert('Delete Successfully');
window.location='state_list.php?msg=updated';
</script>
<?php
}
...
{
$select  = mysqli_query($con,"select * from `state` where `id`='$_REQUEST[e_id]' ")or die(mysqli_error($con));
...
<?php include('top.php'); ?>
		...
		<h2>Create State <span style="float: right;"><a href="state_list.php">State List</a></span></h2>
		<br />
				...
				<div class="panel panel-primary" data-collapsed="0">
					...
					<div class="panel-body">
					    ...
					    ?>
							...
							<div class="form-group">
								<label for="field-1" class="col-sm-3 control-label">State Name <span style="color:#ee4749">*</span></label>
								
								<div class="col-sm-5">
									<input type="text" class="form-control" id="field-1" name="name" placeholder="State Name" required="required" value="<?php echo $select1[name]; ?>">
								</div>
							</div>
