...
{
$select  = mysqli_query($con,"delete from `admin` where `id`='$_REQUEST[d_id]' ")or die(mysqli_error($con));
?>
...
<div class="page-container"><!-- add class "sidebar-collapsed" to close sidebar by default, "chat-visible" to make chat appear always -->
	...
	<div class="main-content">
		...
		<h2>Admin List <span style="float: right;"><a href="create_admin.php">Add Admin</a></span></h2>
		...
		<table class="table table-bordered datatable" id="table-4">
			...
			<tbody>
				...
				$select  = mysqli_query($con,"select * from `admin` where `type`='1' order by `id` desc ")or die(mysqli_error($con));
				$i='1';
				...
				<tr class="odd gradeX">
					...
					<td><?php echo $select1[location]; ?></td>
					<td><?php echo $select1[status]; ?></td>
					<td class="center"><?php echo date('d-m-Y',strtotime($select1[c_date])); ?></td>
					<td class="sorting_1">
						<a href="create_admin.php?e_id=<?php echo $select1[id]; ?>" class="btn btn-default btn-sm btn-icon icon-left">
							<i class="entypo-pencil"></i>
							Edit
						</a>
						<?php if($select1[type]=='1'){ ?>
						<a href="javascript:confirmDelete('admin_list.php?d_id=<?php echo $select1[id]; ?>')" class="btn btn-danger btn-sm btn-icon icon-left">
