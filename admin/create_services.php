...
$qry_validate = mysqli_query($con,"select * from `services` where `name`='$user' ") or die(mysqli_error($con));
if(mysqli_num_rows($qry_validate)>0)
{
$msg ='<center><span style="color:#ff0000;">Services Already Exists<span></center>';
}
else
{
...
$destination1 ='../services/'.$imagename1;
	        
move_uploaded_file($source_image1,$destination1);

mysqli_query($con,"update `services` set `image`='$imagename1' where `id`='$last'");
...
$msg ='<center><span style="color:#ff0000;">Services Insert Successfully<span></center>';
}
...
$update = mysqli_query($con,"update `services` set `name`='$name',`short_description`='$short_description',`order`='$order',`status`='$status' where `id`='$_REQUEST[edit_id]' ")or die(mysqli_error($con));
...
mysqli_query($con,"update `services` set `image`='$imagename1' where `id`='$_REQUEST[edit_id]'");
...
window.location='services_list.php?msg=updated';
</script>
<?php
}
...
{
$select  = mysqli_query($con,"select * from `services` where `id`='$_REQUEST[e_id]' ")or die(mysqli_error($con));
...
<?php include('top.php'); ?>
		...
		<h2>Create Services <span style="float: right;"><a href="services_list.php">Services List</a></span></h2>
		<br />
			...
			<div class="col-md-12">
				...
				<form role="form" class="form-horizontal form-groups-bordered" action="" method="POST" enctype="multipart/form-data">
							...
							<div class="form-group">
								<label for="field-1" class="col-sm-3 control-label">services Name <span style="color:#ee4749">*</span></label>
								
								<div class="col-sm-5">
									<input type="text" class="form-control" id="field-1" name="name" placeholder="services Name"  value="<?php echo $select1[name]; ?>">
								</div>
							</div>
							...
							<div class="form-group">
								<label for="field-1" class="col-sm-3 control-label">services Image <span style="color:#ee4749">*</span></label>
								...
								<div class="col-sm-5">
									<input type="number" class="form-control" id="field-1" name="order" placeholder="services order" required="required" value="<?php echo $select1[order]; ?>" min="0">
								</div>
							</div>
