...
$qry_validate = mysqli_query($con,"select * from `city` where `name`='$name' ") or die(mysqli_error($con));
if(mysqli_num_rows($qry_validate)>0)
{
$msg ='<center><span style="color:#ff0000;">city already exists<span></center>';
}
else
{

$insert = mysqli_query($con,"insert into `city`(`name`,`order`,`status`,`c_date`)values('$name','$order','$status','$c_date')") or die(mysqli_error($con));

$msg ='<center><span style="color:#ff0000;">city insert successfully<span></center>';
}
...
$update = mysqli_query($con,"update `city` set `c_id`='$c_id',`s_id`='$s_id',`name`='$name',`order`='$order',`status`='$status' where `id`='$_REQUEST[edit_id]' ")or die(mysqli_error($con));

//$msg ='<center><span style="color:#ff0000;">Admin Updated Successfully<span></center>';
?>
<script type="text/javascript">
//alert('Delete Successfully');
window.location='city_list.php?msg=updated';
</script>
<?php
}
...
{
$select  = mysqli_query($con,"select * from `city` where `id`='$_REQUEST[e_id]' ")or die(mysqli_error($con));
...
<?php include('top.php'); ?>
		...
		<h2>Create City <span style="float: right;"><a href="city_list.php">City List</a></span></h2>
		<br />
...
<?php } ?>
							...
							<div class="form-group">
								<label for="field-1" class="col-sm-3 control-label">City Name <span style="color:#ee4749">*</span></label>
								
								<div class="col-sm-5">
									<input type="text" class="form-control" id="field-1" name="name" placeholder="City Name" required="required" value="<?php echo $select1[name]; ?>">
								</div>
							</div>
